...
def append():
    ...
    s["kor"] = getScore("국어",limit = 100)
    s["eng"] = getScore("영어",limit = 100) 
    s["mat"] = getScore("수학",limit = 100)    
    s["total"] = getTotal(s)
    s["avg"] = getAvg(s)
    s["grade"] = getGrade(s["avg"])
    scoreList.append(s)
...
def isDigit(s):
    ...
    return s.isdigit()  # s.isdigit()은 명확하게 "문자열이 숫자로만 이루어졌는가?"를 표현.
                        # ord()를 써서 문자 비교를 하는 방식은 직관적이지 않습니다.
...
def delete():
    ...
    if len(resultList)==0:
        print("없는 이름입니다.")
        return 
    print(f"{key}의 점수를 삭제합니다.")
    scoreList.remove(resultList[0])
    output(resultList)
